...
class BoardController: NSObject,
    ...
    func resetBoardWithCurrentSettings() {
      // Step 3: Reset the board without changing the settings
      numTimesGuessed = 0
      // Optionally, if there is any other state that should be reset, reset it here
      collectionView.reloadData()
    }
    ...
    private func applyNumLettersSettings(with settings: [String: Any]) {
        if let numLetters = settings[kNumLettersKey] as? Int {
            ...
            // Adjust the collectionView layout if necessary
            // collectionView.reloadData() // May need to reload data if the view is already loaded
        }
    }
    ...
    private func applyThemeSettings(with settings: [String: Any]) {
        if let themeString = settings[kWordThemeKey] as? String,
           let theme = WordTheme(rawValue: themeString) {
            ...
            // Reload data if necessary, to reflect the new theme
        }
    }
    ...
    private func applyIsAlienWordleSettings(with settings: [String: Any]) {
        if let isAlien = settings[kIsAlienWordleKey] as? Bool {
            ...
            // Here you might need to set up something that changes the word after each guess
        }
    }
}
